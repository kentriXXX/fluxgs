...
local function trace(step)
    if FLUX_TRACE then
        client.error_log('flux: ' .. step)
    end
end

trace('stage 0 - chunk started')
...
-- FIX: the original script calls toticks()/totime() but never defines them
local function toticks(time)
    return math.floor((time or 0) / globals.tickinterval() + 0.5)
end
...
trace('stage 1 - libraries loaded')
...
local script do
    ...
    script.user = 'user' do
        local ok, persona = pcall(function ()
    ...
    script.discord  = 'https://dsc.gg/luaflux'
    script.telegram = 'https://t.me/luaflux'

    script.discord_short  = 'https://dsc.gg/luaflux'
    script.telegram_short = 'https://t.me/luaflux'
end
...
trace('stage 2 - low level (color/utils/globalvars/fs/surface)')
...
trace('stage 3 - software references')
...
local config_system do
    ...
    function config_system.on_export(callback)
        table.insert(export_callbacks, callback)
    end
    ...
    function config_system.decode(str)
        -- flux strings first, flux exports still import fine
        local data = str:match('%[flux%] (.-)_')
            or str:match('%[flux%] (.-)_')
    ...
    function config_system.export(categories)
        ...
        for i = 1, #export_callbacks do
            export_callbacks[i](list, categories)
        end
...
local menu_logic do
    ...
    function menu_logic.lock_updates()
        update_locked = update_locked + 1
    end

    function menu_logic.unlock_updates()
        update_locked = math.max(0, update_locked - 1)

        if update_locked == 0 and update_pending then
            menu_logic.update()
        end
    end

    function menu_logic.update(item)
        pending_item = item or pending_item

        if is_updating or update_locked > 0 then
            update_pending = true
        ...
        repeat
            ...
            local changed_item = pending_item
            pending_item = nil

            is_updating = true
            logic_events.update:fire(changed_item)
    ...
    local menu_events = menu.get_event_bus() do
        ...
        local function on_item_changed(item)
            menu_logic.update(item)
        end
...
local text_anims do
    ...
    -- flux: cosmic sweep (violet -> blue -> cyan), used by the indicator style
    function text_anims.cosmic(str, time, h, s, v, scale)
        local list = { }

        local strbuf, strlen = u8(str)
        local div = 1 / math.max(strlen - 1, 1)
        local col = color()

        local step = 0

        for i = 1, strlen do
            local wave = math.sin((time * 2) + step * 3.4) * 0.5 + 0.5

            -- 0.72 violet .. 0.52 cyan
            col:hsv(0.72 - wave * 0.20, 0.55 + wave * 0.35, 1.0)

            table.insert(list, '\a')
            table.insert(list, col:to_hex())
            table.insert(list, strbuf[i])
...
trace('stage 4 - menu / menu_logic / config system')
...
local localplayer do
    ...
    localplayer.is_defensive_peeking = false
    ...
    -- from @enq
    local function is_peeking(player, prediction_ticks)
        local should, vulnerable = false, false
        local velocity = vector(entity.get_prop(player, 'm_vecVelocity'))

        local eye = vector(client.eye_position())
        local peye = utils.extrapolate(eye, velocity, prediction_ticks or 14)
        ...
        for i = 1, #enemies do
            ...
            if entindex == enemy and damage ~= nil and damage > 0 then
                should = true
                break
            end
    ...
    local function on_setup_command(cmd)
        ...
        local predicted_peeking = is_peeking(me, 16)
        local defensive_peeking = peeking or predicted_peeking
        ...
        -- FIX: exported so other modules (safe head) can read them
        localplayer.pre_flags = pre_flags
        localplayer.post_flags = post_flags

        localplayer.is_peeking = peeking
        localplayer.is_defensive_peeking = defensive_peeking
...
trace('stage 5 - localplayer / exploit / statement')

local menu_animation_controller do
    menu_animation_controller = { }
    ...
    local function log_error(message)
        client.error_log('Flux menu animations: ' .. message)
    end
    ...
    local function resolve_winapi()
        ...
        if not jmp_ecx or not get_proc_call or not get_module_call then
            return nil, 'WinAPI resolver signatures failed'
        end
        ...
        if kernel32 == 0 or virtual_protect == 0 or virtual_alloc == 0 then
            return nil, 'VirtualProtect/VirtualAlloc resolve failed'
        end
    ...
    local function site_accepts_patch(address, original_bytes, patch)
        if bytes_equal(address, original_bytes) or bytes_equal(address, patch) then
            return true
        end
    ...
    local function create_patch_sites()
        return {
            {
                name = 'select',
    ...
    -- eased (smoothstep) progress: the fade starts slow, speeds up in the
    -- middle and settles at the end, so switching tabs feels smooth instead
    -- of a constant-speed fade
    local tab_elapsed = 0
    local content_elapsed = 0

    local function ease_in_out(t)
        return t * t * (3 - 2 * t)
    end

    local function eased_progress(elapsed, duration_ms)
        local duration = duration_ms * 0.001

        if elapsed >= duration then
            return MAX_ALPHA
        end

        local value = math.floor(ease_in_out(elapsed / duration) * MAX_ALPHA + 0.5)

        if value < 1 then
            value = 1
        end

        if value > MAX_ALPHA then
            value = MAX_ALPHA
        end
    ...
    local function update_animation()
        ...
        if tab_just_reset or content_just_reset then
            ...
            if tab_just_reset then
                tab_elapsed = 0
            end

            if content_just_reset then
                content_elapsed = 0
            end
        ...
        if current_tab_progress < MAX_ALPHA then
            tab_elapsed = tab_elapsed + dt
            tab_progress[0] = eased_progress(tab_elapsed, TAB_TIME_MS)
        end

        if current_content_progress < MAX_ALPHA then
            content_elapsed = content_elapsed + dt
            content_progress[0] = eased_progress(content_elapsed, CONTENT_TIME_MS)
        end

        for i = 0, HOVER_SLOT_COUNT - 1 do
            if hover_down[i] == 0 and hover_alpha[i] > HOVER_BASE_ALPHA then
                hover_alpha[i] = subtract_hover_alpha(hover_alpha[i], TAB_TIME_MS, dt)
            end
        end
    ...
    local function initialize()
        ...
        local ok, success, message = pcall(function()
            local resolved_winapi, resolve_error = resolve_winapi()

            if resolved_winapi == nil then
                return false, resolve_error
            end

            local allocated_cave = resolved_winapi.alloc(CAVE_SIZE)

            if allocated_cave == nil then
                return false, 'VirtualAlloc failed'
            end

            local allocated_base = tonumber(cast('uintptr_t', allocated_cave))

            if allocated_base == nil or allocated_base == 0 then
                return false, 'VirtualAlloc failed'
            end

            winapi = resolved_winapi
            cave_ptr = allocated_cave
            cave_base = allocated_base
            region = {
                select = cave_base,
            ...
            state = cast('uint32_t*', region.state)
            tab_progress = cast('uint32_t*', state_addr.progress)
            content_progress = cast('uint32_t*', state_addr.content_progress)
            hover_down = cast('uint32_t*', state_addr.hover_down)
            hover_alpha = cast('uint32_t*', state_addr.hover_alpha)
            ...
            for i = 1, #renderer_hooks do
                renderer_hooks[i].original = nil
                renderer_hooks[i].wrapper = nil
            end

            reset_state()

            local cave_code = make_cave_code()
            ...
            patch_sites = create_patch_sites()
            patched = false
            last_time = globals.realtime()
            last_tab_progress = MAX_ALPHA
            last_content_progress = MAX_ALPHA
        ...
        if not ok then
            initialized = false
            patched = false
            log_error('initialization failed: ' .. tostring(success))
            return false
        end
        ...
        if not success then
            initialized = false
            patched = false
            log_error(message or 'initialization failed')
            return false
        end
    ...
    function menu_animation_controller.set_enabled(value)
        if not value then
            restore_patch()
            return true
        end
    ...
    function menu_animation_controller.shutdown()
        restore_patch()
    end
end

local jumpscout_fix_controller do
    jumpscout_fix_controller = { }

    local cast = ffi.cast

    local PATCH_ADDR = 0x43372D89
    local PAGE_EXECUTE_READWRITE = 0x40

    local original_bytes = ffi.new('uint8_t[13]', {
        0xF7, 0x40, 0x04, 0x00, 0x00, 0x02, 0x00,
        0x0F, 0x84, 0x94, 0x04, 0x00, 0x00
    })

    local patch_bytes = ffi.new('uint8_t[13]')
    ffi.fill(patch_bytes, ffi.sizeof(patch_bytes), 0x90)

    local initialized = false
    local callbacks_registered = false
    local enabled = false
    local owns_patch = false
    local winapi
    local old_protection = ffi.new('unsigned long[1]')

    local function log_error(message)
        client.error_log('Flux jumpscout fix: ' .. tostring(message))
    end

    local function find_signature(module, pattern)
        local address = client.find_signature(module, pattern)
        ...
        return tonumber(cast('uintptr_t', address))
    end
    ...
    local function resolve_winapi()
        ...
        if kernel32 == 0 then
            return nil, 'kernel32.dll resolve failed'
        end

        local virtual_protect = call_get_proc_address(get_proc_address, 0, kernel32, 'VirtualProtect')

        if virtual_protect == 0 then
            return nil, 'VirtualProtect resolve failed'
        end
    ...
    local function apply_patch()
        local size = ffi.sizeof(original_bytes)

        if bytes_match(PATCH_ADDR, patch_bytes, size) then
            return true
        end

        if owns_patch then
            owns_patch = false
            log_error('patch ownership lost')
            return false
        end

        if not bytes_match(PATCH_ADDR, original_bytes, size) then
            log_error('byte check failed')
            return false
        end

        if not write_bytes(PATCH_ADDR, patch_bytes, size) then
    ...
    local function restore_patch()
        if not initialized or not owns_patch then
            return
        end
        ...
        if bytes_match(PATCH_ADDR, patch_bytes, size) then
            if not write_bytes(PATCH_ADDR, original_bytes, size) then
                log_error('patch restore failed')
                return
            end
        end
    ...
    local function should_patch()
        local local_player = entity.get_local_player()

        if not local_player or local_player == 0 or not entity.is_alive(local_player) then
            return false
        end

        return entity.get_prop(local_player, 'm_bIsScoped') == 1
    end

    local function update_patch()
        if not enabled or not should_patch() then
            restore_patch()
            return true
        end
    ...
    local function register_callbacks()
        ...
        client.set_event_callback('setup_command', update_patch)
        client.set_event_callback('paint', update_patch)
        callbacks_registered = true
    end
    ...
    function jumpscout_fix_controller.set_enabled(value)
        if not value then
            enabled = false
            restore_patch()
            return true
    ...
    function jumpscout_fix_controller.shutdown()
        enabled = false
        restore_patch()
    end
end
...
-- ---------------------------------------------------------------- session statistics
trace('stage 6 - animation + jumpscout controllers')

local session_stats do
    session_stats = { }

    local KEY = 'flux_stats'
    local stored = database.read(KEY)
    ...
    session_stats.total = {
        kills   = tonumber(stored.kills)   or 0,
        deaths  = tonumber(stored.deaths)  or 0,
        assists = tonumber(stored.assists) or 0,
        hours   = tonumber(stored.hours)   or 0
    }

    session_stats.session = { kills = 0, deaths = 0, assists = 0 }

    local started_at = globals.realtime()
    local accounted = 0
    local last_flush = globals.realtime()

    local function uptime()
        return globals.realtime() - started_at
    end

    local function ratio(kills, deaths)
        return deaths > 0 and kills / deaths or kills
    end
    ...
    function session_stats.flush()
        local elapsed = uptime() - accounted

        accounted = uptime()
        last_flush = globals.realtime()
        session_stats.total.hours = session_stats.total.hours + elapsed / 3600

        database.write(KEY, {
            kills   = session_stats.total.kills,
            deaths  = session_stats.total.deaths,
            assists = session_stats.total.assists,
            hours   = session_stats.total.hours
        })
    end

    function session_stats.tick()
        if globals.realtime() - last_flush > 60 then
            session_stats.flush()
        end
    end

    function session_stats.reset()
        session_stats.session = { kills = 0, deaths = 0, assists = 0 }
        session_stats.total = { kills = 0, deaths = 0, assists = 0, hours = 0 }

        accounted = uptime()
        session_stats.flush()
    end

    session_stats.ref = nil

    function session_stats.display()
        local ref = session_stats.ref

        if ref == nil then
            return
        end
        local s, total = session_stats.session, session_stats.total

        ref.session:set(string.format('Session: %d kills / %d deaths / %d assists', s.kills, s.deaths, s.assists))
        ref.session_kd:set(string.format('Session K/D: %.2f', ratio(s.kills, s.deaths)))
        ref.total:set(string.format('Total: %d kills / %d deaths / %d assists', total.kills, total.deaths, total.assists))
        ref.total_kd:set(string.format('Total K/D: %.2f', ratio(total.kills, total.deaths)))
        ref.hours:set(string.format('Playtime: %.1f h', total.hours + (uptime() - accounted) / 3600))
        ref.uptime:set(string.format('Uptime: %s', format_uptime()))
    end
    ...
    client.set_event_callback('player_death', function (e)
        ...
        local attacker = e.attacker ~= 0 and client.userid_to_entindex(e.attacker) or nil
        local assister = e.assister ~= nil and e.assister ~= 0 and client.userid_to_entindex(e.assister) or nil

        if attacker == me and victim ~= me then
            session_stats.session.kills = session_stats.session.kills + 1
            session_stats.total.kills = session_stats.total.kills + 1
        end

        if victim == me then
            session_stats.session.deaths = session_stats.session.deaths + 1
            session_stats.total.deaths = session_stats.total.deaths + 1
        end

        if assister == me then
            session_stats.session.assists = session_stats.session.assists + 1
            session_stats.total.assists = session_stats.total.assists + 1
        end
    end)
...
local resource do
    ...
    -- FIX: a multiselect returns ONE table with every selected option, so
    -- { item:get() } was always a table of size 1 (even with nothing selected)
    -- and the "at least one option" guard never ran
    local function get_selection(item)
    ...
    local function lock_unselection(item, default_value)
        local old_value = get_selection(item)
        ...
        if #old_value == 0 then
            ...
            item:set(unpack(default_value))
        end

        item:set_callback(function()
            local value = get_selection(item)

            if #value > 0 then
                old_value = value
            else
                item:set(unpack(old_value))
            end
        end)
    end
    ...
    local ok_general, err_general = pcall(function ()
        -- ---------------------------------------------------------------- accent colours
        -- the whole header theme is driven by these two pickers (violet -> blue by default)
        local function new_accent(name, key, r, g, b)
            local ok, item = pcall(function ()
                return config_system.push(
                    'Interface', 'general.' .. key, menu.new(
                        ui.new_color_picker, 'AA', 'Fake lag', name, r, g, b, 255
                    )
                )
            end)
            ...
            client.error_log('flux: colour picker [' .. name .. '] failed -> ' .. tostring(item))

            -- fallback so the rest of the menu keeps working
            return { get = function () return r, g, b, 255 end, set_callback = function () end }
        end

        general.accent_from = new_accent('Accent', 'accent_from', 138, 92, 255)
        general.accent_to   = new_accent('Accent 2', 'accent_to', 77, 124, 255)

        trace('stage 7.a accent colour pickers')

        local function accent_hex()
            return utils.to_hex(general.accent_from:get())
        end

        -- ---------------------------------------------------------------- animated wordmark
        -- "flux nightly" painted with an accent gradient and a highlight travelling over it
        trace('stage 7.b wordmark helpers')

        local WORDMARK = string.format('%s ⁓ %s', script.name, script.build)

        -- IMPORTANT: a menu item name has a hard length limit in gamesense. One colour code per
        -- character would produce a ~120 byte name and kill the script without a lua error, so
        -- text is painted in a few segments and the result is always capped.
        local GRADIENT_LIMIT = 56
        local GRADIENT_MAX_SEGMENTS = 4

        -- utf8 aware: the swung dash is three bytes, a byte split would corrupt it
        local char_cache = { }

        local function to_chars(text)
            local cached = char_cache[text]
            ...
            char_cache[text] = chars
            return chars
        end

        -- one animated violet -> blue gradient with a travelling highlight, shared by the
        -- wordmark and by every "Flux ⁓ <page>" header
        local function build_gradient(text, phase)
            local r1, g1, b1, a1 = general.accent_from:get()
            local r2, g2, b2, a2 = general.accent_to:get()

            local chars = to_chars(text)
            local count = #chars

            -- keep the final string inside the name limit: 9 bytes per colour code
            local segments = math.floor((GRADIENT_LIMIT - #text) / 9)

            if segments > GRADIENT_MAX_SEGMENTS then segments = GRADIENT_MAX_SEGMENTS end
            if segments > count then segments = count end

            if segments < 2 then
                return string.format('\a%02x%02x%02x%02x%s', r1, g1, b1, a1, text)
            end

            local per_segment = math.ceil(count / segments)

            local out = { }
            local index = 1

            for segment = 1, segments do
                local chunk = table.concat(chars, '', index, math.min(index + per_segment - 1, count))

                if chunk == '' then
                    break
                end

                local f = segments > 1 and (segment - 1) / (segments - 1) or 0
                ...
                -- travelling glow
                local wave = math.sin(f * 4.0 - phase)
                local glow = wave > 0 and wave * wave * wave or 0

                r = r + (255 - r) * glow * 0.85
                g = g + (255 - g) * glow * 0.85
                b = b + (255 - b) * glow * 0.85

                out[#out + 1] = string.format('\a%02x%02x%02x%02x%s',
                    math.floor(r), math.floor(g), math.floor(b), math.floor(a), chunk
                )

                index = index + per_segment
            ...
            if #result > GRADIENT_LIMIT + 4 then
                result = string.format('\a%02x%02x%02x%02x%s', r1, g1, b1, a1, text)
            end

            return result
        end

        local function build_wordmark(phase)
            return build_gradient(WORDMARK, phase)
        end

        local function get_script_name_label()
            return build_wordmark(0)
        end

        local function get_script_username_label()
            return string.format('\a%s%s', accent_hex(), script.user)
        end

        trace('stage 7.c wordmark built')

        general.script_name = menu.new(
            ui.new_label, 'AA', 'Fake lag', WORDMARK
        )

        trace('stage 7.c1 wordmark label created')

        pcall(function ()
            general.script_name:set(get_script_name_label())
        end)

        trace('stage 7.c2 wordmark text applied')

        general.script_user = menu.new(
            ui.new_label, 'AA', 'Fake lag', script.user
        )
        ...
        trace('stage 7.d script labels')

        -- flux gs style navigation: one flat Menu combobox, plus a page selector for the
        -- tabs that carry more than one screen
        general.tabs = {
            global  = 'Global',
            ragebot = 'Ragebot',
            antiaim = 'Anti-aim',
        ...
        general.tab = menu.new(
            ui.new_combobox, 'AA', 'Fake lag', new_key('Menu', 'navigation'), {
                general.tabs.global,
                general.tabs.ragebot,
                general.tabs.antiaim,
                general.tabs.visuals,
                general.tabs.misc,
                general.tabs.config
            }
        )

        trace('stage 7.e menu combobox')

        general.page_antiaim = menu.new(
            ui.new_combobox, 'AA', 'Fake lag', new_key('Page', 'antiaim'), {
                'Builder', 'Defensive', 'Features', 'Hotkeys'
            }
        )

        general.page_visuals = menu.new(
            ui.new_combobox, 'AA', 'Fake lag', new_key('Page', 'visuals'), {
                'Changers', 'Interface'
            }
        )

        general.page_misc = menu.new(
            ui.new_combobox, 'AA', 'Fake lag', new_key('Page', 'misc'), {
                'Miscellaneous', 'Autobuy'
            }
        )

        trace('stage 7.f page comboboxes')

        -- the scene below still dispatches on the original numeric categories,
        -- 15 = Global and 16 = Stats are the two new flux pages
        local CATEGORY = {
            [general.tabs.global]  = 15,
            [general.tabs.ragebot] = 0,
            [general.tabs.config]  = 14
        }

        local PAGE_ANTIAIM = { ['Builder'] = 6, ['Defensive'] = 17, ['Features'] = 7, ['Hotkeys'] = 8 }
        local PAGE_VISUALS = { ['Changers'] = 10, ['Interface'] = 11 }
        local PAGE_MISC    = { ['Miscellaneous'] = 1, ['Autobuy'] = 4 }

        function general.get_page_item()
            local tab = general.tab:get()

            if tab == general.tabs.antiaim then return general.page_antiaim end
            if tab == general.tabs.visuals then return general.page_visuals end
            if tab == general.tabs.misc    then return general.page_misc end

            return nil
        end

        function general.get_category()
            local tab = general.tab:get()

            if tab == general.tabs.antiaim then
                return PAGE_ANTIAIM[general.page_antiaim:get()] or 6
            end

            if tab == general.tabs.visuals then
                return PAGE_VISUALS[general.page_visuals:get()] or 10
            end

            if tab == general.tabs.misc then
                return PAGE_MISC[general.page_misc:get()] or 1
            end

            return CATEGORY[tab] or 15
        end

        trace('stage 7.g category maps')

        -- ---------------------------------------------------------------- page header
        -- every page carries the same "Flux ⁓ <page>" title, the scene just retitles it
        local PAGE_TITLES = {
            [0]  = 'Ragebot',
            [1]  = 'Miscellaneous',
            [4]  = 'Autobuy',
        ...
        -- 'flux' -> 'Flux', every page header reads "Flux ⁓ <page>"
        local SCRIPT_TITLE = script.name:sub(1, 1):upper() .. script.name:sub(2)

        local function page_text(title)
            return string.format('%s ⁓ %s', SCRIPT_TITLE, title)
        end

        local function page_title(title, phase)
            return build_gradient(page_text(title), phase or 0)
        end

        general.page_header = menu.new(
            ui.new_label, 'AA', 'Anti-aimbot angles', page_title('Global')
        )

        general.page_separator = menu.new(
            ui.new_label, 'AA', 'Anti-aimbot angles', '\n'
        )

        local current_title = 'GLOBAL'

        function general.update_page_header(category)
            current_title = PAGE_TITLES[category] or 'Menu'
            general.page_header:set(page_title(current_title))
        end

        function general.refresh_titles()
            general.page_header:set(page_title(current_title))
            general.stats.header:set(page_title('STATS'))
            general.script_user:set(get_script_username_label())
        end

        -- everything that glows, animated from the scene paint handler
        function general.update_animation()
            local phase = globals.realtime() * 3

            general.script_name:set(build_wordmark(phase))
            general.page_header:set(page_title(current_title, phase + 0.8))
            general.stats.header:set(page_title('STATS', phase + 1.6))
        end

        -- retitle everything as soon as a colour is changed
        general.accent_from:set_callback(function () general.refresh_titles() end)
        general.accent_to:set_callback(function () general.refresh_titles() end)

        trace('stage 7.h page header')

        -- ---------------------------------------------------------------- Global page
        general.global = { } do

            general.global.build = menu.new(
                ui.new_label, 'AA', 'Anti-aimbot angles', string.format('Build: %s', script.build)
            )

            general.global.user = menu.new(
                ui.new_label, 'AA', 'Anti-aimbot angles', string.format('User: %s', script.user)
            )

            general.global.discord = menu.new(
                ui.new_label, 'AA', 'Anti-aimbot angles', 'discord: ' .. script.discord_short
            )

            general.global.telegram = menu.new(
                ui.new_label, 'AA', 'Anti-aimbot angles', 'telegram: ' .. script.telegram_short
            )

            general.global.copy_discord = menu.new(
                ui.new_button, 'AA', 'Anti-aimbot angles', 'Copy Discord link', function ()
                    clipboard.set(script.discord)
                    logging.success('Discord link copied.')
                end
            )

            general.global.copy_telegram = menu.new(
                ui.new_button, 'AA', 'Anti-aimbot angles', 'Copy Telegram link', function ()
                    clipboard.set(script.telegram)
                    logging.success('Telegram link copied.')
                end
            )

            general.global.separator = menu.new(
                ui.new_label, 'AA', 'Anti-aimbot angles', '\n'
            )
        end

        -- ---------------------------------------------------------------- Stats page
        trace('stage 7.i global page')

        -- statistics sit on the Global page, in the right hand column
        general.stats = { } do
            general.stats.header = menu.new(
                ui.new_label, 'AA', 'Other', page_title('STATS')
            )

            general.stats.session    = menu.new(ui.new_label, 'AA', 'Other', 'Session: 0 kills / 0 deaths')
            general.stats.session_kd = menu.new(ui.new_label, 'AA', 'Other', 'Session K/D: 0.00')
            general.stats.total      = menu.new(ui.new_label, 'AA', 'Other', 'Total: 0 kills / 0 deaths')
            general.stats.total_kd   = menu.new(ui.new_label, 'AA', 'Other', 'Total K/D: 0.00')
            general.stats.hours      = menu.new(ui.new_label, 'AA', 'Other', 'Playtime: 0.0 h')
            general.stats.uptime     = menu.new(ui.new_label, 'AA', 'Other', 'Uptime: 00:00:00')

            general.stats.reset = menu.new(
                ui.new_button, 'AA', 'Other', 'Reset statistics', function ()
                    if general.stats.on_reset ~= nil then
                        general.stats.on_reset()
                    end
                end
            )

            general.stats.separator = menu.new(
        ...
        trace('stage 7.j stats page')

        session_stats.ref = general.stats

        general.stats.on_reset = function ()
            session_stats.reset()
            session_stats.display()

            logging.success('Statistics reset.')
        end
    ...
    if not ok_general then
        client.error_log('flux: menu/general block failed -> ' .. tostring(err_general))
    end

    trace('stage 7.1 - general menu ok')
    ...
    local main = { } do
        local ragebot = { } do
            ...
            local dt_bug_fix = { } do
                dt_bug_fix.enabled = config_system.push(
                    'Ragebot', 'dt_bug_fix.enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('DT bug fix', 'dt_bug_fix')
                    )
                )

                ragebot.dt_bug_fix = dt_bug_fix
            end
            ...
            local jumpscout_fix = { } do
                jumpscout_fix.enabled = config_system.push(
                    'Ragebot', 'jumpscout_fix.enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Jumpscout', 'jumpscout')
                    )
                )

                jumpscout_fix.enabled:set_callback(function(item)
                    local value = item:get()

                    if not jumpscout_fix_controller.set_enabled(item:get()) and value then
                        item:set(false)
                    end
                end, true)

                ragebot.jumpscout_fix = jumpscout_fix
            end

            local hitscan_fix = { } do
                hitscan_fix.enabled = config_system.push(
                    'Ragebot', 'hitscan_fix.enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Hitscan fix', 'hitscan_fix')
                    )
                )

                hitscan_fix.spread_samples = config_system.push(
                    'Ragebot', 'hitscan_fix.spread_samples', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('HS spread samples', 'hitscan_fix')
                    )
                )

                hitscan_fix.sample_count = config_system.push(
                    'Ragebot', 'hitscan_fix.sample_count', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('HS sample count', 'hitscan_fix'), 128, 512, 512
                    )
                )

                hitscan_fix.fixed_seed_order = config_system.push(
                    'Ragebot', 'hitscan_fix.fixed_seed_order', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('HS fixed seed order', 'hitscan_fix')
                    )
                )

                hitscan_fix.percent_cap = { } do
                    hitscan_fix.percent_cap.enabled = config_system.push(
                        'Ragebot', 'hitscan_fix.percent_cap.enabled', menu.new(
                            ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('HC percent cap', 'hitscan_fix')
                        )
                    )

                    hitscan_fix.percent_cap.value = config_system.push(
                        'Ragebot', 'hitscan_fix.percent_cap.value', menu.new(
                            ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('HC max percent', 'hitscan_fix'), 1, 100, 76, true, '%'
                        )
                    )
                end

                hitscan_fix.hitbox_cap = { } do
                    hitscan_fix.hitbox_cap.enabled = config_system.push(
                        'Ragebot', 'hitscan_fix.hitbox_cap.enabled', menu.new(
                            ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('HS hitbox cap', 'hitscan_fix')
                        )
                    )

                    hitscan_fix.hitbox_cap.value = config_system.push(
                        'Ragebot', 'hitscan_fix.hitbox_cap.value', menu.new(
                            ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('HS hitboxes', 'hitscan_fix'), 1, 19, 18
                        )
                    )
                end

                hitscan_fix.hitbox_mask = { } do
                    hitscan_fix.hitbox_mask.enabled = config_system.push(
                        'Ragebot', 'hitscan_fix.hitbox_mask.enabled', menu.new(
                            ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('HS hitbox mask', 'hitscan_fix')
                        )
                    )

                    hitscan_fix.hitbox_mask.value = config_system.push(
                        'Ragebot', 'hitscan_fix.hitbox_mask.value', menu.new(
                            ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('HS mask value', 'hitscan_fix'), 1, 255, 255
                        )
                    )
                end

                hitscan_fix.distance_bypass = config_system.push(
                    'Ragebot', 'hitscan_fix.distance_bypass', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('HS distance mask bypass', 'hitscan_fix')
                    )
                )

                hitscan_fix.spread_samples:set(true)
                hitscan_fix.fixed_seed_order:set(true)
                hitscan_fix.percent_cap.enabled:set(true)
                hitscan_fix.hitbox_cap.enabled:set(true)
                hitscan_fix.hitbox_mask.enabled:set(true)
                hitscan_fix.distance_bypass:set(true)

                ragebot.hitscan_fix = hitscan_fix
            end

            local danilya_fast_switch = { } do
                danilya_fast_switch.enabled = config_system.push(
                    'Ragebot', 'danilya_fast_switch.enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Danilya fast switch', 'danilya_fast_switch')
                    )
                )

                ragebot.danilya_fast_switch = danilya_fast_switch
            end
        ...
        local miscellaneous = { } do
            ...
            local trash_talk = { } do
                ...
                -- both triggers are always selected, the multiselect can never be emptied
                lock_unselection(trash_talk.triggers, { 'On Kill', 'On Death' })
        ...
        local animations = { } do
            animations.air_legs = config_system.push(
                'Animations', 'anim_breaker.air_legs', menu.new(
                    ui.new_combobox, 'AA', 'Other', new_key('Air legs', 'animations'), {
                        'Off',
                        'Static',
                        'Moonwalk',
                        'Kangaroo'
            ...
            animations.air_legs_weight = config_system.push(
                'Animations', 'anim_breaker.air_legs_weight', menu.new(
                    ui.new_slider, 'AA', 'Other', new_key('Weight', 'animations'), 0, 100, 100, true, '%'
                )
            )

            animations.ground_legs = config_system.push(
                'Animations', 'anim_breaker.ground_legs', menu.new(
                    ui.new_combobox, 'AA', 'Other', new_key('Ground legs', 'animations'), {
                        'Off',
                        'Static',
                        'Jitter',
                        'Moonwalk',
            ...
            animations.move_lean = config_system.push(
                'Animations', 'anim_breaker.move_lean', menu.new(
                    ui.new_slider, 'AA', 'Other', new_key('Move lean', 'animations'), -1, 100, -1, true, '', 1, {
                        [-1] = 'Off'
                    }
                )
            )
        ...
        local logging_system = { } do
            ...
            logging_system.enabled = config_system.push(
                'Hitlogs', 'logging_system.enabled', menu.new(
                    ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Hitlogs', 'logging_system')
                )
            )

            logging_system.events = config_system.push(
                'Hitlogs', 'logging_system.events', menu.new(
            ...
            logging_system.output = config_system.push(
                'Hitlogs', 'logging_system.output', menu.new(
            ...
            logging_system.events_font = config_system.push(
                'Hitlogs', 'logging_system.events_font', menu.new(
            ...
            logging_system.offset_y = config_system.push(
                'Hitlogs', 'logging_system.offset_y', menu.new(
            ...
            logging_system.duration = config_system.push(
                'Hitlogs', 'logging_system.duration', menu.new(
                    ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Duration', 'logging_system'), 5, 40, 20, true, 's.', 0.1
                )
            )

            logging_system.console_text_style = config_system.push(
                'Hitlogs', 'logging_system.console_text_style', menu.new(
            ...
            logging_system.crosshair_text_style = config_system.push(
                'Hitlogs', 'logging_system.crosshair_text_style', menu.new(
            ...
            for i = 1, #main_color_list do
                ...
                items.color = config_system.push(
                    'Hitlogs', string.format('logging_system.%s', color_key), menu.new(
        ...
        local autobuy = { } do
            autobuy.enabled = config_system.push(
                'Autobuy', 'buy_bot.enabled', menu.new(
                    ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Enabled', 'buy_bot')
                )
            )

            autobuy.primary = config_system.push(
                'Autobuy', 'buy_bot.primary', menu.new(
            ...
            autobuy.alternative = config_system.push(
                'Autobuy', 'buy_bot.alternative', menu.new(
            ...
            autobuy.secondary = config_system.push(
                'Autobuy', 'buy_bot.secondary', menu.new(
            ...
            autobuy.equipment = config_system.push(
                'Autobuy', 'buy_bot.equipment', menu.new(
            ...
            autobuy.ignore_pistol_round = config_system.push(
                'Autobuy', 'buy_bot.ignore_pistol_round', menu.new(
                    ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Ignore pistol round', 'buy_bot')
                )
            )

            autobuy.only_16k = config_system.push(
                'Autobuy', 'buy_bot.only_16k', menu.new(
                    ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Only $16k', 'buy_bot')
                )
            )

            main.autobuy = autobuy
        end
    ...
    local antiaim = { } do
        local builder = { } do
            local function has_yaw_directions(state)
                return state == 'Moving'
                    or state == 'Air'
                    or state == 'Air-Crouch'
                    or state == 'Move-Crouch'
            end

            local function create_defensive_items(state, team, direction, should_save, is_directional)
                local items = { }

                local function hash(key)
                    local value = state .. ':' .. team .. ':defensive_' .. key
                ...
                local function add(key, item)
                    ...
                    return config_system.push('Builder', hash(key), item)
                end

                if is_directional then
                    items.override = menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key(
                            'Override defensive', hash 'override'
                        )
                    )
                end

                -- everything below lives in the same column as the state / team selectors
                items.force_defensive = add('force_defensive', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key(
                ...
                items.enabled = add('enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key(
                ...
                items.pitch = add('pitch', menu.new(
                ...
                items.pitch_offset_1 = add('pitch_offset_1', menu.new(
                ...
                items.pitch_offset_2 = add('pitch_offset_2', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('\n', hash 'pitch_offset_2'), -89, 89, 0, true, '°'
                    )
                )

                items.pitch_speed = add('pitch_speed', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Speed', hash 'pitch_speed'), -75, 75, 20, true, nil, 0.1
                    )
                )

                items.yaw = add('yaw', menu.new(
                ...
                items.ways_count = add('ways_count', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('\n', hash 'ways_count'), 3, 7, 3, true, ''
                    )
                )

                items.ways_custom = add('ways_custom', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Custom ways', hash 'ways_custom')
                    )
                )

                for i = 1, 7 do
                    items['way_' .. i] = add('way_' .. i, menu.new(
                            ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('\n', hash('way_' .. i)), -180, 180, 0, true, '°'
                        )
                    )
                end

                items.yaw_offset = add('yaw_offset', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('\n', hash 'yaw_offset'), -180, 180, 0, true, '°'
                    )
                )

                items.yaw_left = add('yaw_left', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Yaw left', hash 'yaw_left'), -180, 180, 0, true, '°'
                    )
                )

                items.yaw_right = add('yaw_right', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Yaw right', hash 'yaw_right'), -180, 180, 0, true, '°'
                    )
                )

                items.yaw_speed = add('yaw_speed', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Speed', hash 'yaw_speed'), -75, 75, 20, true, '', 0.1
                    )
                )

                items.yaw_random = add('yaw_random', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Yaw random', hash 'yaw_random'), 0, 30, 0, true, '%'
                    )
                )

                items.ways_auto_body_yaw = add('ways_auto_body_yaw', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Automatic body yaw', hash 'ways_auto_body_yaw')
                    )
                )

                items.body_yaw = add('body_yaw', menu.new(
                ...
                items.body_yaw_offset = add('body_yaw_offset', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('\n', hash 'body_yaw_offset'), -180, 180, 0, true, '°'
                    )
                )

                items.body_yaw_random = add('body_yaw_random', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Body yaw random', hash 'body_yaw_random'), 0, 30, 0, true, '%'
                    )
                )

                items.freestanding_body_yaw = add('freestanding_body_yaw', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key(
                ...
                items.delay_from = add('delay_from', menu.new(
                ...
                items.delay_to = add('delay_to', menu.new(
                ...
                items.invert_chance = add('invert_chance', menu.new(
            ...
            local function get_defensive_values(items)
                local values = { }

                for key, item in pairs(items) do
                    if item.type ~= 'label' and item.get ~= nil then
                        values[key] = item:get()
                    end
                end
            ...
            local function set_defensive_values(items, values)
                for key, item in pairs(items) do
                    local value = values[key]

                    if item.type ~= 'label' and value ~= nil then
                        pcall(item.set, item, value)
                    end
                end
            end

            -- the shared directional editor is built at the very end of this block so
            -- that its menu items are published UNDER the state / team selectors
            -- instead of above them (menu items keep their creation order)
            local directional_defensive = {
                editor = nil,
                active = nil,
                updating = false,
                instances = { }
            ...
            local function copy_directional_values(values)
                local copy = { }
                ...
                for direction, profile in pairs(values) do
                    if type(profile) == 'table' then
                        local copied_profile = { }

                        for key, value in pairs(profile) do
                            copied_profile[key] = value
                        end

                        copy[direction] = copied_profile
                    end
                end
            ...
            -- One editor is shared by every state/team direction profile.
            local function create_directional_defensive_items(state, team)
                local items = {
                    editor = nil,
                    current = nil,
                    cache = { },
                    cache_raw = nil,
                    key = state .. ':' .. team .. ':defensive_direction'
                }

                table.insert(directional_defensive.instances, items)

                items.store = config_system.push(
                    'Builder', items.key, menu.new(
                        ui.new_string, items.key, ''
                    )
                )

                function items:load_cache()
                    local raw = self.store:get() or ''

                    if self.cache_raw == raw then
                        return false
                    end
                    ...
                    local ok, values = pcall(json.parse, raw)
                ...
                function items:get_values(direction)
                    self:load_cache()

                    local values = self.cache[direction]
                ...
                function items:save_current()
                    if directional_defensive.active ~= self
                        or directional_defensive.updating
                        or self.current == nil then
                        return
                    end

                    self.cache[self.current] = get_defensive_values(
                        directional_defensive.editor
                    )
                end

                function items:load_editor(fallback)
                    self:load_cache()

                    local values = self.cache[self.current]

                    if type(values) ~= 'table' then
                        values = get_defensive_values(fallback)
                        values.override = false
                    end

                    directional_defensive.updating = true
                    menu_logic.lock_updates()
                    set_defensive_values(directional_defensive.editor, values)
                    directional_defensive.updating = false
                    menu_logic.unlock_updates()
                end

                function items:activate(direction, fallback)
                    local active = directional_defensive.active
                    ...
                    if active ~= self then
                        ...
                        directional_defensive.active = self
                    ...
                    if self.current ~= direction then
                        self:save_current()
                        self.current = direction
                        self:load_editor(fallback)
                ...
                function items:deactivate()
                    if directional_defensive.active == self then
                        self:save_current()
                        directional_defensive.active = nil
                    end

                    self.current = nil
                end

                function items:get_proxy(direction, fallback)
                    local values = self:get_values(direction)

                    if values == nil or not values.override then
                        return nil
                    end

                    local proxy = { }

                    for key, item in pairs(fallback) do
                        if item.type ~= 'label' and item.get ~= nil then
                            local value = values[key]
                            local fallback_item = item
                ...
                function items:serialize()
                    self:save_current()

                    local ok, raw = pcall(json.stringify, self.cache)

                    if ok then
                        return raw
                    end

                    return self.store:get() or ''
                end

                function items:export()
                    self:load_cache()
                    self:save_current()

                    return copy_directional_values(self.cache)
                end

                function items:import(values, fallback)
                    if type(values) ~= 'table' then
                        return
                    end

                    self.cache = copy_directional_values(values)
                ...
                function items:copy_direction_to(
                    target, direction, source_fallback, target_fallback
                )
                    self:load_cache()
                    self:save_current()
                    ...
                    if type(values) == 'table' then
                        values = copy_directional_values({
                            [direction] = values
                        })[direction]
                    else
                        values = get_defensive_values(source_fallback or { })
                    end

                    -- A copied direction must use its own defensive profile.
                    values.override = true
                    target.cache[direction] = values

                    local ok, raw = pcall(json.stringify, target.cache)

                    if not ok then
                        return false
                    end

                    target.cache_raw = raw
                    target.store:set(raw)

                    if directional_defensive.active == target
                        and target.current == direction then
                        target:load_editor(target_fallback)
                    end

                    return true
                end

                config_system.on_export(function(data)
                    local builder_data = data.Builder

                    if builder_data ~= nil then
                        builder_data[items.key] = { items:serialize() }
                    end
                end)
            ...
            local function create_builder_items(state, team, std_key)
                ...
                if not is_freestanding then
                    if has_yaw_directions(state) then
                            ...
                            items['yaw_left_dir_' .. dir] = config_system.push(
                                ...
                                items['yaw_right_dir_' .. dir] = config_system.push(
                                    'Builder', hash('yaw_right_dir_' .. dir), menu.new(
                                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key('Yaw right', hash('yaw_right_dir_' .. dir)), -180, 180, 0, true, '°'
                                    )
                                )
            ...
            local function get_values(items)
                ...
                if items.defensive_directions ~= nil then
                    data.defensive_directions = items.defensive_directions:export()
                end
            ...
            local function set_values(items, data)
                ...
                if items.defensive_directions ~= nil then
                    items.defensive_directions:import(
                        data.defensive_directions, items.defensive
                    )
                end
            ...
            local function copy_direction_values(source, target, direction)
                local source_angles = source.angles
                local target_angles = target.angles
                local keys = {
                    'enabled_dir_' .. direction,
                    'yaw_left_dir_' .. direction,
                    'yaw_right_dir_' .. direction
                }

                menu_logic.lock_updates()

                if target_angles.enabled ~= nil then
                    target_angles.enabled:set(true)
                end

                if target_angles.yaw_direction ~= nil then
                    target_angles.yaw_direction:set(direction)
                end

                for i = 1, #keys do
                    local key = keys[i]
                    local source_item = source_angles[key]
                    local target_item = target_angles[key]

                    if source_item ~= nil and target_item ~= nil then
                        if key == 'enabled_dir_' .. direction then
                            target_item:set(true)
                        else
                            target_item:set(source_item:get())
                        end
                    end
                end

                if source.defensive_directions ~= nil
                    and target.defensive_directions ~= nil then
                    source.defensive_directions:copy_direction_to(
                        target.defensive_directions,
                        direction,
                        source.defensive,
                        target.defensive
                    )
            ...
            for i = 1, #const.states do
                ...
                items.team = menu.new(
                    -- FIX: one unique key per state, every state used to create a
                    -- 'Team' combobox under the exact same menu name
                    ui.new_combobox, 'AA', 'Anti-aimbot angles', new_key('Team', 'builder:' .. state), const.teams
                )
                ...
                for j = 1, #const.teams do
                    ...
                    if state ~= 'Fakelag' then
                        ...
                        if has_yaw_directions(state) then
                            team_items.defensive_directions = create_directional_defensive_items(
                                state, team
                            )
                        end
                    end
                    ...
                    team_items.separator = menu.new(
                        ui.new_label, 'AA', 'Anti-aimbot angles', new_key('\n', 'separator:' .. state .. ':' .. team)
                    )

                    team_items.send_to_another_team = menu.new(
                        ui.new_button, 'AA', 'Anti-aimbot angles', new_key(
                            'Send to another team', 'builder:' .. state .. ':' .. team
                        ), function()
                            ...
                            local direction = team_items.angles.yaw_direction
                                and team_items.angles.yaw_direction:get()

                            if direction ~= nil and direction ~= 'General' then
                                copy_direction_values(
                                    team_items, target, direction
                                )

                                logging.success(string.format(
                                    'sent %s direction to another team', direction
                                ))
                            ...
                            logging.success('sent state to another team')
                        end
                    )
            ...
            -- built last on purpose: the editor rows show up below the state / team
            -- selectors and below the per-state defensive rows
            directional_defensive.editor = create_defensive_items(
                'Directional', 'Editor', nil, false, true
            )

            for _, item in pairs(directional_defensive.editor) do
                if item.type ~= 'label' then
                    item:set_callback(function()
                        local active = directional_defensive.active
            ...
            for i = 1, #directional_defensive.instances do
                directional_defensive.instances[i].editor = directional_defensive.editor
            end
        ...
        local hotkeys = { } do
            ...
            local real_yaw_arrows = { } do
                real_yaw_arrows.enabled = config_system.push(
                    'Hotkeys', 'real_yaw_arrows.enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key(
                            'Real yaw arrow', 'real_yaw_arrows'
                        )
                    )
                )

                real_yaw_arrows.offset = config_system.push(
                    'Hotkeys', 'real_yaw_arrows.offset', menu.new(
                        ui.new_slider, 'AA', 'Anti-aimbot angles', new_key(
                            'Arrow radius', 'real_yaw_arrows'
                        ), 8, 128, 40, true, 'px'
                    )
                )

                real_yaw_arrows.color = config_system.push(
                    'Hotkeys', 'real_yaw_arrows.color', menu.new(
                        ui.new_color_picker, 'AA', 'Anti-aimbot angles', new_key(
                            'Arrow color', 'real_yaw_arrows'
                        ), 35, 128, 255, 255
                    )
                )

                real_yaw_arrows.separator = menu.new(
                    ui.new_label, 'AA', 'Anti-aimbot angles', '\n'
                )

                hotkeys.real_yaw_arrows = real_yaw_arrows
            end
        ...
        local fakelag = { } do
            ...
            fakelag.limit = menu.new(
                ui.new_slider, 'AA', 'Other', new_key('Limit', 'fakelag'), 1, 24, 1, true, 't'
            )

            fakelag.force_choke = config_system.push(
                'Features', 'fakelag.force_choke', menu.new(
                    ui.new_checkbox, 'AA', 'Other', new_key('Force choke', 'fakelag')
                )
            )

            fakelag.unsafe_choke = config_system.push(
                'Features', 'fakelag.unsafe_choke', menu.new(
                    ui.new_checkbox, 'AA', 'Other', new_key('Unsafe choke', 'fakelag')
                )
            )
        ...
        local changers = { } do
            ...
            local custom_scope = { } do
                ...
                custom_scope.style = config_system.push(
                    'Interface', 'custom_scope.style', menu.new(
                ...
                custom_scope.exclude = config_system.push(
                    'Interface', 'custom_scope.exclude', menu.new(
                ...
                custom_scope.animation_speed = config_system.push(
                    'Interface', 'custom_scope.animation_speed', menu.new(
        ...
        local user_interface = { } do
            local watermark = { } do
                watermark.select = config_system.push(
                    'Interface', 'watermark.select', menu.new(
                ...
                watermark.accent_color = config_system.push(
                    'Interface', 'watermark.accent_color', menu.new(
                ...
                watermark.removals = config_system.push(
                    'Interface', 'watermark.removals', menu.new(
                ...
                watermark.position = config_system.push(
                    'Interface', 'watermark.position', menu.new(
            ...
            local indicators = { } do
                ...
                indicators.select = config_system.push(
                    'Interface', 'indicators.select', menu.new(
                ...
                indicators.offset = config_system.push(
                    'Interface', 'indicators.offset', menu.new(
                ...
                indicators.accent_color = config_system.push(
                    'Interface', 'indicators.accent_color', menu.new(
                ...
                indicators.secondary_color = config_system.push(
                    'Interface', 'indicators.secondary_color', menu.new(
            ...
            local damage_indicator = { } do
                damage_indicator.enabled = config_system.push(
                    'Interface', 'damage_indicator.enabled', menu.new(
                ...
                damage_indicator.offset = config_system.push(
                    'Interface', 'damage_indicator.offset', menu.new(
                ...
                damage_indicator.active_color = config_system.push(
                    'Interface', 'damage_indicator.active_color', menu.new(
                ...
                damage_indicator.inactive_color = config_system.push(
                    'Interface', 'damage_indicator.inactive_color', menu.new(
            ...
            local menu_animations = { } do
                menu_animations.enabled = config_system.push(
                    'Interface', 'menu_animations.enabled', menu.new(
                        ui.new_checkbox, 'AA', 'Anti-aimbot angles', new_key('Menu Animations', 'menu_animations')
                    )
                )

                menu_animations.enabled:set_callback(function(item)
                    if not menu_animation_controller.set_enabled(item:get()) then
                        item:set(false)
                    end
                end, true)

                user_interface.menu_animations = menu_animations
            end
        ...
        local hit_markers = { } do
            local damage_marker = { } do
                ...
                damage_marker.body_color = config_system.push(
                    'Interface', 'damage_marker.body_color', menu.new(
                ...
                damage_marker.head_color = config_system.push(
                    'Interface', 'damage_marker.head_color', menu.new(
                ...
                damage_marker.mismatch_color = config_system.push(
                    'Interface', 'damage_marker.mismatch_color', menu.new(
            ...
            local world_marker = { } do
                world_marker.enabled = config_system.push(
                    'Interface', 'world_marker.enabled', menu.new(
                ...
                world_marker.horizontal_color = config_system.push(
                    'Interface', 'world_marker.horizontal_color', menu.new(
                ...
                world_marker.vertical_color = config_system.push(
                    'Interface', 'world_marker.vertical_color', menu.new(
    ...
    local scene do
        ...
        -- section = 'angles' (Builder page) or 'defensive' (Defensive page)
        local function update_builder_items(items, section)
            local show_angles    = section ~= 'defensive'
            local show_defensive = section ~= 'angles'

            local angles = items.angles
            local defensive = items.defensive
            local defensive_directions = items.defensive_directions
            local selected_direction = 'General'
            local is_selected_direction_enabled = false

            if items.separator ~= nil and show_angles then
                menu_logic.set(items.separator, true)
            end

            if items.send_to_another_team ~= nil and show_angles then
                menu_logic.set(items.send_to_another_team, true)
            end

            -- The angle part still has to run on the Defensive page, it resolves which yaw
            -- direction is selected. Its items are simply not published there.
            local restore_set = menu_logic.set

            if not show_angles then
                menu_logic.set = function () end
            end
            ...
            if angles ~= nil then
                if angles.enabled ~= nil then
                    ...
                    if not angles.enabled:get() then
                        -- FIX: the stub above must always be restored, otherwise
                        -- menu_logic.set stays dead for the whole session and no
                        -- menu item is ever published again (menu looks frozen)
                        menu_logic.set = restore_set
                ...
                if angles.yaw_left ~= nil and angles.yaw_right ~= nil then
                    ...
                    if angles.yaw_direction ~= nil then
                        ...
                        is_selected_direction_enabled = item_enable:get()

                        if not is_selected_direction_enabled then
                            goto continue
                        end
            ...
            -- the direction selector stays on the Defensive page: it picks what is edited
            if not show_angles and angles ~= nil and angles.yaw_direction ~= nil then
                menu_logic.set(angles.yaw_direction, true)
            end

            if items.separator_1 ~= nil and show_angles then
                menu_logic.set(items.separator_1, true)
            end

            if defensive_directions ~= nil and angles ~= nil and angles.yaw_direction ~= nil then
                if selected_direction ~= 'General' and is_selected_direction_enabled then
                    defensive_directions:activate(selected_direction, defensive)

                    local editor = defensive_directions.editor
                    menu_logic.set(editor.override, true)

                    if editor.override:get() then
                        defensive = editor
                    end
                else
                    defensive_directions:deactivate()
                end
            end

            if defensive ~= nil and show_defensive then
                ...
                if defensive.enabled:get() then
                    ...
                    if defensive.yaw:get() ~= 'Off' then
                        ...
                        menu_logic.set(defensive.yaw_random, true)
                    end
                    ...
                    if defensive.body_yaw:get() ~= 'Off' then
                        if defensive.body_yaw:get() ~= 'Opposite' then
                            ...
                            menu_logic.set(defensive.body_yaw_random, true)
                        end
        ...
        local function force_update_scene()
            ...
            local category = general.get_category()

            menu_logic.set(general.tab, true)
            menu_logic.set(general.get_page_item(), true)

            -- Flux ⁓ <page> on top of every page
            general.update_page_header(category)

            menu_logic.set(general.page_header, true)
            menu_logic.set(general.page_separator, true)

            -- Global
            if category == 15 then
                menu_logic.set(general.global.build, true)
                menu_logic.set(general.global.user, true)
                menu_logic.set(general.global.discord, true)
                menu_logic.set(general.global.telegram, true)
                menu_logic.set(general.global.copy_discord, true)
                menu_logic.set(general.global.copy_telegram, true)
                menu_logic.set(general.global.separator, true)
            end

            -- Statistics: part of the Global page, right hand column
            if category == 15 then
                menu_logic.set(general.stats.header, true)
                menu_logic.set(general.stats.session, true)
                menu_logic.set(general.stats.session_kd, true)
                menu_logic.set(general.stats.total, true)
                menu_logic.set(general.stats.total_kd, true)
                menu_logic.set(general.stats.hours, true)
                menu_logic.set(general.stats.uptime, true)
                menu_logic.set(general.stats.reset, true)
                menu_logic.set(general.stats.separator, true)
            end
            ...
            if category == 0 then
                ...
                menu_logic.set(ref.dt_bug_fix.enabled, true)
                menu_logic.set(ref.hideshots_fix.enabled, true)
                menu_logic.set(ref.jumpscout_fix.enabled, true)

                menu_logic.set(ref.hitscan_fix.enabled, true)

                menu_logic.set(ref.danilya_fast_switch.enabled, true)
            end
            ...
            if category == 1 then
                ...
                local is_fps_optimize = ref.fps_optimize.enabled:get() do
                    ...
                    -- always on: optimizations are applied whenever the feature is enabled
                ...
                local is_clantag = ref.clantag.enabled:get() do
                    ...
                    if ref.clantag.text:get() == 'Flux' then
                    end
            ...
            -- Animations: part of the Miscellaneous page now
            if category == 1 then
            ...
            -- Hitlogs: part of the Miscellaneous page now
            if category == 1 then
                ...
                local is_colors_visible = (
                    ref.console_text_style:get() == 'New' or
                    ref.crosshair_text_style:get() == 'New'
                )
            ...
            -- Autobuy
            if category == 4 then
                local ref = resource.main.autobuy
            ...
            -- Builder
            -- Builder (angles) and Defensive share the state / team selectors
            if category == 6 or category == 17 then
                local builder do
                    ...
                    update_builder_items(
                        team_items, category == 17 and 'defensive' or 'angles'
                    )
            ...
            if category == 7 then
                ...
                local fakelag do
                    ...
                    menu_logic.set(ref.force_choke, true)
                    menu_logic.set(ref.unsafe_choke, true)
                end
            end
            ...
            if category == 8 then
                ...
                local real_yaw_arrows = ref.real_yaw_arrows
                menu_logic.set(real_yaw_arrows.enabled, true)

                if real_yaw_arrows.enabled:get() then
                    menu_logic.set(real_yaw_arrows.offset, true)
                    menu_logic.set(real_yaw_arrows.color, true)
                    menu_logic.set(real_yaw_arrows.separator, true)
                end
            ...
            if category == 11 then
                ...
                menu_logic.set(ref.menu_animations.enabled, true)
                ...
                local watermark do
                    ...
                    if ref.watermark.select:get 'Full' then
                    ...
                    if ref.watermark.select:get 'Split' then
            ...
            -- Hit markers: part of the Interface page now
            if category == 11 then
        ...
        local function on_shutdown()
            menu_animation_controller.shutdown()
            jumpscout_fix_controller.shutdown()
        ...
        local function on_paint_ui()
            local category = resource.general.get_category()

            if ui.is_menu_open() then
                resource.general.update_animation()
            end
...
trace('stage 8 - resource (menu built)')
...
local config do
    ...
    local config_defaults = {
        [1] = {
            name = 'Default',
            data = '[flux] _'
        },
    }
    ...
    local callbacks do
        ...
        local function on_load()
            ...
            -- no category filter: a config always carries every section
        ...
        local function on_import()
            ...
            config_system.import(result, nil)
...
trace('stage 9 - windows / config / ex_render')
...
local main do
    local rage do
        ...
        local dt_bug_fix do
            local ref = resource.main.ragebot.dt_bug_fix
            ...
            local patches = {
                ...
                {
                    address = 0x433ABFFE,
                    original = { 0x0F, 0x85, 0x9A, 0x02, 0x00, 0x00 },
                    patched = { 0x0F, 0x84, 0x9A, 0x02, 0x00, 0x00 }
                },
                {
                    address = 0x433AC0F7,
                    original = { 0x0F, 0x85, 0xA1, 0x02, 0x00, 0x00 },
                    patched = { 0x0F, 0x84, 0xA1, 0x02, 0x00, 0x00 }
                },
                {
                    address = 0x433AC167,
                    original = { 0x0F, 0x86, 0x31, 0x01, 0x00, 0x00 },
                    patched = { 0xE9, 0x32, 0x01, 0x00, 0x00, 0x90 }
                },
                {
                    address = 0x433ABEA5,
                    original = { 0x0F, 0x8E, 0xF3, 0x03, 0x00, 0x00 },
                    patched = { 0xE9, 0xF4, 0x03, 0x00, 0x00, 0x90 }
                }
            }

            local active = false

            local function bytes_match(patch, bytes)
                local pointer = ffi.cast('uint8_t*', patch.address)

                for i = 1, #bytes do
                    if pointer[i - 1] ~= bytes[i] then
                        return false
                    end
                end
            ...
            local function write_bytes(patch, bytes)
                local pointer = ffi.cast('uint8_t*', patch.address)
            ...
            local function apply_patches()
                ...
                for i = 1, #patches do
                    ...
                    if not bytes_match(patch, patch.original) then
                        return
                    end
                end
                ...
                for i = 1, #patches do
                    local patch = patches[i]
                    write_bytes(patch, patch.patched)
                end
            ...
            local function restore_patches()
                ...
                for i = 1, #patches do
                    ...
                    if bytes_match(patch, patch.patched) then
                        write_bytes(patch, patch.original)
                    end
                end
        ...
        local hitscan_fix do
            local ref = resource.main.ragebot.hitscan_fix

            local function to_u8(value)
                return math.floor(value) % 0x100
            end

            local function to_le32(value)
            ...
            local function join(first, second)
                ...
                for i = 1, #first do
                    bytes[#bytes + 1] = first[i]
                end

                for i = 1, #second do
                    bytes[#bytes + 1] = second[i]
                end

                return bytes
            end

            local function cmp_imm32(address, register, original)
                return {
                    kind = 'cmp_imm32',
                    address = address,
                    register = register,
                    original = join({ 0x81, register }, to_le32(original)),
                    build = function(value)
                        return join({ 0x81, register }, to_le32(value))
                    end
                }
            end

            local function push_imm8(address, original)
                return {
                    kind = 'push_imm8',
                    address = address,
                    original = { 0x6A, original },
                    build = function(value)
                        return { 0x6A, to_u8(value) }
                    end
                }
            end

            local function mov_esi_imm32(address, original)
                return {
                    kind = 'mov_esi_imm32',
                    address = address,
                    original = join({ 0xBE }, to_le32(original)),
                    build = function(value)
                        return join({ 0xBE }, to_le32(value))
                    end
                }
            end

            local function disp32(address, prefix, original)
                return {
                    kind = 'disp32',
                    address = address,
                    prefix = prefix,
                    original = join(prefix, to_le32(original)),
                    build = function(value)
                        return join(prefix, to_le32(value))
                    end
                }
            end

            local function bytes(address, original, patched)
                return {
                    kind = 'bytes',
                    address = address,
                    original = original,
            ...
            local sample_patches = {
                cmp_imm32(0x4333278D, 0xFF, 0x80),
                cmp_imm32(0x4333285D, 0xF9, 0x80)
            }

            local seed_redirect_patches = {
                disp32(0x433326CF, { 0x0F, 0xB6, 0x88 }, 0x4346FC50),
                disp32(0x4333273E, { 0x0F, 0xB6, 0x87 }, 0x4346FC50),
                disp32(0x433327B3, { 0x0F, 0xB6, 0x91 }, 0x4346FC50),
                disp32(0x4333281F, { 0x0F, 0xB6, 0x80 }, 0x4346FC50)
            }

            local percent_cap_patches = {
                push_imm8(0x43332890, 0x64)
            }

            local hitbox_cap_patches = {
                push_imm8(0x433337EA, 0x13),
                push_imm8(0x43333C88, 0x13)
            }

            local hitbox_mask_patches = {
                mov_esi_imm32(0x43333349, 0xCA)
            }

            local distance_bypass_patches = {
                bytes(0x43333078, { 0x76, 0x27 }, { 0xEB, 0x27 }),
                bytes(0x433330A8, { 0x76, 0x0D }, { 0xEB, 0x0D })
            }

            local groups = {
                sample = sample_patches,
                seed = seed_redirect_patches,
                percent_cap = percent_cap_patches,
                hitbox_cap = hitbox_cap_patches,
                hitbox_mask = hitbox_mask_patches,
                distance_bypass = distance_bypass_patches
            }

            local fixed_seed_table = ffi.new('unsigned char[512]')

            for i = 0, 511 do
                fixed_seed_table[i] = i % 256
            end

            local fixed_seed_address = nil
            local has_seed_address, seed_address = pcall(function()
                return tonumber(ffi.cast(
                    'uintptr_t', ffi.cast('void*', fixed_seed_table)
                ))
            end)

            if has_seed_address and seed_address >= 0 and seed_address <= 0xFFFFFFFF then
                fixed_seed_address = seed_address
            else
                local has_fallback_address, fallback_address = pcall(function()
                    return tonumber(ffi.cast(
                        'unsigned int', ffi.cast('void*', fixed_seed_table)
                    ))
                end)

                if has_fallback_address
                    and fallback_address >= 0
                    and fallback_address <= 0xFFFFFFFF then
                    fixed_seed_address = fallback_address
                end
            end

            local function get_fixed_seed_address()
                fixed_seed_table[0] = fixed_seed_table[0]
                return fixed_seed_address
            end

            local function read_bytes(address, length)
                local result = { }
                local success = pcall(function()
                    local pointer = ffi.cast('unsigned char*', address)

                    for i = 1, length do
                        result[i] = pointer[i - 1]
                    end
                end)
            ...
            local function write_bytes(address, values)
                return pcall(function()
                    ...
                    for i = 1, #values do
                        pointer[i - 1] = values[i]
                    end
                end)
            end

            local function matches(first, second)
            ...
            local function is_patch_valid(patch)
                local current = read_bytes(patch.address, #patch.original)

                if matches(current, patch.original) then
                    return true
                end

                if patch.kind == 'bytes' then
                    return matches(current, patch.patched)
                end
                ...
                if patch.kind == 'cmp_imm32' then
                    return current[1] == 0x81 and current[2] == patch.register
                end

                if patch.kind == 'push_imm8' then
                    return current[1] == 0x6A
                end

                if patch.kind == 'mov_esi_imm32' then
                    return current[1] == 0xBE
                end

                for i = 1, #patch.prefix do
                    if current[i] ~= patch.prefix[i] then
                        return false
                    end
                end
            ...
            local function is_group_valid(group)
                for i = 1, #group do
                    if not is_patch_valid(group[i]) then
                        return false
                    end
                end
            ...
            local valid = fixed_seed_address ~= nil

            for _, group in pairs(groups) do
                valid = valid and is_group_valid(group)
            end

            local active = { }
            local values = { }

            local function apply_group(group, value)
                for i = 1, #group do
                    local patch = group[i]
                    local patched = patch.kind == 'bytes'
                        and patch.patched
                        or patch.build(value)

                    if not write_bytes(patch.address, patched) then
                        return false
                    end
                end
            ...
            local function restore_group(name)
                ...
                for i = 1, #group do
                    local patch = group[i]
                    write_bytes(patch.address, patch.original)
                end
            ...
            local function update_group(name, enabled, value)
                if not enabled then
                    restore_group(name)
                    return
                end

                if active[name] and values[name] == value then
                    return
                end

                if apply_group(groups[name], value) then
                    active[name] = true
                    values[name] = value
                end
            end
            ...
            local duck_peek_active = nil
            local airborne = nil
            local post_landing_ticks = 0
            local last_sample_count = nil

            local function get_sample_count()
                ...
                if not localplayer.is_onground
                    or post_landing_ticks > 0 then
                    return 256
                end
            ...
            local function update()
                ...
                local samples = get_sample_count()
                local use_fixed_seed = ref.fixed_seed_order:get()
                    or (samples ~= nil and samples > 128)

                update_group('seed', use_fixed_seed, get_fixed_seed_address())
                update_group('sample', samples ~= nil, samples)
                update_group(
                    'percent_cap',
                    ref.percent_cap.enabled:get(),
                    ref.percent_cap.value:get()
                )
                update_group(
                    'hitbox_cap',
                    ref.hitbox_cap.enabled:get(),
                    ref.hitbox_cap.value:get()
                )
                update_group(
                    'hitbox_mask',
                    ref.hitbox_mask.enabled:get(),
                    ref.hitbox_mask.value:get()
                )
                update_group(
                    'distance_bypass', ref.distance_bypass:get(), true
                )
            end

            local function on_setup_command()
                local is_active = software.is_duck_peek_assist()
                local is_airborne = not localplayer.is_onground
                local was_airborne = airborne == true

                if is_airborne or was_airborne then
                    post_landing_ticks = 4
                end

                local samples = get_sample_count()
                local should_update = duck_peek_active ~= is_active
                    or airborne ~= is_airborne
                    or last_sample_count ~= samples

                duck_peek_active = is_active
                airborne = is_airborne
                last_sample_count = samples

                if should_update and ref.enabled:get()
                    and ref.spread_samples:get() then
                    update()
                end

                if not is_airborne and post_landing_ticks > 0 then
                    post_landing_ticks = post_landing_ticks - 1
                end
            end
            ...
            local callbacks do
                local function on_value_changed()
                    update()
                end

                ref.enabled:set_callback(on_value_changed, true)
                ref.spread_samples:set_callback(on_value_changed)
                ref.sample_count:set_callback(on_value_changed)
                ref.fixed_seed_order:set_callback(on_value_changed)

                ref.percent_cap.enabled:set_callback(on_value_changed)
                ref.percent_cap.value:set_callback(on_value_changed)

                ref.hitbox_cap.enabled:set_callback(on_value_changed)
                ref.hitbox_cap.value:set_callback(on_value_changed)

                ref.hitbox_mask.enabled:set_callback(on_value_changed)
                ref.hitbox_mask.value:set_callback(on_value_changed)

                ref.distance_bypass:set_callback(on_value_changed)
            end
        ...
        local danilya_fast_switch do
            local ref = resource.main.ragebot.danilya_fast_switch
    ...
    local miscellaneous do
        local fps_optimize do
            ...
            -- always on: convars stay applied while the feature is enabled
        ...
        local trash_talk do
            ...
            local MESSAGES = {
                ['On Kill'] = {
                    ...
                    {
                        { 'все очень просто', 1.1 },
                        { 'я тебя убил, ты заныл.', 1.8 },
                    },
                    ...
                    {
                        { 'попущен под громкие стоны твоей мамы', 1.6 },
                    },


                    {
                        { 'сфокусируйся на члене молчанием', 1.4 },
                    },
                    ...
                    {
                        { 'куда ползешь', 1 },
                        { 'к червякам в землю?', 1.9 }
                    },

                    {
                        { 'ооо бля ты ждал что я миссну?', 2.1 },
                        { 'в следующий раз батю пну нахуй))', 2.3 }
                    },

                    {
                        { 'загадай самопровокацию слизаную с дика ебыря', 1.4 },
                    },

                    {
                        { 'я твоей маме премию дал за хороший секс))))', 1.6 },
                    },

                    {
                        { 'ИНФУКАЙ В МАКАФОН ХУЙНЯ', 2.1 },
                        { 'НАЙС ИНФААА)', 1.7 }
                    },

                    {
                        { 'мисссааеешшшш??', 2.1 },
                        { 'ГгЕTHи flux NIGHTLYYY))))', 2.6 }
                    },

                    {
                        { 'ало данилЯ?', 1.8 },
                        { 'Да тут про тебя спизданули. я его уебал', 2.5 }
                    },
                    ...
                    {
                        { 'бесполезно против меня играть', 2.1 }
                    },

                    {
                        { 'Твоя мама пошлая МОЛЬ)))', 1.6 },
                    },
                    {
                        { 'Ало полиция? Да тут маньяк с флюксом всех ебет', 1.4 },
                    },

                    {
                        { 'АЛО ЗДРАСЬТЕ, ВЫ КТО??', 1.6 },
                    },
                    ...
                    {
                        { 'МУСОРАБЛЯДИ', 2.2 },
                        { 'я против мусоров в ROBLOX в игре San Diego Border RolePlay', 2.7 }
                    },

                    {
                        { 'Погоди, а это точно белая темка?', 2.1 },
                    },

                    {
                        { 'ЕБАЛ ТВОЮ МАМУ ПОД ГРАФИК СОЛАНЫ)', 1.3 },
                        { '', 1.7 },
                    },

                    {
                        { 'напрасно пикнул друзь', 2.0 }
                    },

                    {
                        { 'HEY YOOOO', 2.1 },
                        { 'THIS IS OWNER TONKEEPER', 2.6 }
                    },

                    {
                        { 'что там хуйня ебанная втопила ебало?', 2.1 }
                    },

                    {
                        { 'червяк ебаный замкадный куда спешишь', 1.5 },
                    },

                    {
                        {'я кончеННЫй', 2.2  },
                        {'но ты прям червь без флюкса + без скита + без нейма', 2.5 }
                    },

                    {
                        { 'таракан, я твою маму раздавил)', 1.3 },
                    },

                    {
                        { 'Что Что ПаЦАаНы сЛыШАли О LAG COMPENSATION? Яя Вот БРиКаЮ)) еЕ', 2.3 },
                    },

                    {
                        { 'Здраствуйте подвердите что вы не робот.', 1.9 },
                    },

                    {
                        { 'дрочил?', 1.2 },
                        { 'на свою маму голую?', 1.8 },
                        { 'я вот дрочил твоей на лицо прям', 2 }
                    },
                    ...
                    {
                        { 'ты опять 1 в ебло словил', 2.3 },
                        { 'парашу свою удали нахуй', 2.8 }
                    },

                    {
                        { 'нормально ты как хуйня гОМаЕШЬ0))', 2.4 }
                    },

                    {
                        { 'я пикнул - мой друг трахнул твою маму', 1.9 }
                    },

                    {
                        { 'повезло мне однако', 1 },
                        { 'что я ебал твою маму 44 раза', 1 }
                    },

                    {
                        { 'пиддорррассс 2026 гооодд а ты как ебанный червь уумираееешш', 2.1 }
                    },

                    {
                        { 'заценииии айдии хуйууйняяяяяяяя', 1.7 },
                        { 'А ДА У ТЕБЯ НЕ БУДЕТ ТАКОГО)))', 2.3 }
                    },
...

                    {
                        { 'ппизда короче', 1 },
                        { 'я админку купил', 1 },
                        { '!ban клоп_ебанный 30лет не умеет играть', 1 },

                    },

                    {
                        { 'опять тапнут?', 2 },
                        { 'попробуй анти аим флюкса хзз', 2.5 }
                    },
                    
                    {
                        { 'алоооооооо хуйняяяяя', 1.1 },
                        { 'ливай с серверааааа', 1.9 },
                    },
                    {
                        { 'жри говно бесконечнооо', 1.5 },
                    },
                    {
                        { 'Ало ты тут?', 2 }
                    },
                    ...
                    {
                        { 'не пикай такое', 2 },
                        { 'лучше доставай нож против меня и стой афк', 2.5 }
                    },

                    {
                        { 'труп ебанный', 1.1 },
                        { 'не пиши ниче в чат', 1.9 },
                    },
                    ...
                    {
                        { 'Я в шоке, это player?', 2.3 }
                    },

                    {
                        { 'бич im хоми and ебыpь твоей m0ther', 1 },
                        { '1', 1.5 }
                    },
                    ...
                    {
                        { 'моя жопа срет на лицо твоей матери', 2 },
                        { 'дай реакцию', 1.6 }
                    },
                    {
                        { 'чистое лц', 1.20 },
                    ...
                    {
                        { 'хуйсосиЧмО', 1.20 },
                        { 'в my flux найтлИ попасть impossible', 1.6 },
                    },

                    {
                        { 'Дым дым казино и твоя мама шлюха ебанная', 1.20 },
                        { 'ЪАЪАЪЪА ПРИКОЛЬНО', 1.20 },
                    },
                    ...
                    {
                        { 'ало вася? можешь помяукать в войс?', 1.20 },
                    },

                    {
                        { 'слышь ты че говно козье, нахуя играешь тут?', 1.6 },
                    },

                    {
                        { 'ало да 5х5 босс это я', 1.2 },
                        { 'ты лишь червяк замкадный', 1.3 },
                    },

                    {
                        { 'бич учись играть зайбал', 1.5 },
                    },

                    {
                        { 'ержай вставай я ебу твою мать', 1.2 },
                    },

                    {
                        { 'MOGGED хуйня прыщавая))', 1.1 },
                    },

                    {
                        { 'Следующая остановка ---> могила сочника', 1.4 },
                    },
                    ...
                    {
                        { 'убьешь меня когда или не?', 2 },
                    },

                    {
                        { 'с дороги нахуй, каха едет ебать твою блядь', 1.4 },
                    },

                    {
                        { 'дрочувсалонеМерса))', 1.3 },
                    },

                    {
                        { 'вафля под шконку быстрее', 1.4 },
                    },
...

                    {
                        { 'короче', 2 },
                        { 'план такой', 1.01 },
                        { 'я ебу твою мать - ты следишь что бы она не сдохла', 1.9 },
                    },

                    {
                        { 'ладно', 1.1 },
                        { 'я пошел качать запрет', 2.1 },
                        { 'с яндекса с самой первой ссылки, там все безопасно!', 1 },
                    },


                    {
                        { 'врубись в темку хуйня', 2 },
                    },

                    {
                        { 'аллахуакбар', 1.5 },
                        { 'мистербист раздает 300 баксов по промокоду jimmy', 1.5 },
                    },

                    {
                        { 'Клоп? не не видал', 1.4 },
                    },

                    {
                        { 'Ты как досочка, также рубацаю те ебло', 1.2 },
                    },

                    {
                        { 'Здарова хач ебанный!', 1.1 },
                    },
                    ...
                    },
                    {
                        ...
                        { 'flux nightly бы купил на скит бомж', 2.5 }
                    },
                    {
                        { 'хахаха это ты называешь игрой', 1.9 },
                        { 'иди в майнкрафт играй с читом лучше', 2.1 }
                    },
                    {
                        { 'ало пацаны, дифуза на пла мета?', 2.2 },
                        { 'в патче 7.41a', 2.4 }
                    }
                },
                ...
                ['On Death'] = {
                    {
                        { '!pause', 2.1 },
                        { 'поддержите паузу я спать', 2.4 },
                    },

                    {
                        { 'все я сплю не мешай', 2.1 }
                    },

                    {
                        { 'я разъебал монитор', 3 },
                        { 'плати, или иду в ментовку', 3 }
                    },
                    ...
                    {
                        { 'НУ ПОЧЕМУ МЕНЯ УБИВАЕТ ПИДОРАС С ДЛЛКОЙ', 2 },
                        { 'ХОТЯ У МЕНЯ ОРИГ СКИТ ЗАЕБАЛИ', 2.3 },
                    },
                    

                    {
                        { 'СУКАААААА', 2.3 },
                        { 'подскажи где купил луашку на предикт', 2.5 },
                        { 'не у повротика случаем?)', 2 }
                    },

                    {
                        { 'как там пацаны в эмберлаше', 2 },
                        { 'он же, не понимает, что ему повезло, так ведь?', 2 }
                    },

                    {
                        { '2 ЧАСТЬ СКОРО ПАЦАНЫ НА ЮТЮБЕ ГРОМКИЕ СТОНЫ ТВОЕЙ МАМЫ', 2 }
                    },

                    {
                        { 'че радуешься?', 2 },
                        { 'радуйся твоему селу пизда в него ракета летит', 3.4 }
                    },

                    {
                        { 'НУ СУКА ОПЯТЬ МЫШКУ СВОДИТ', 3 },
                        { '', 4 }
                    },

                    {
                        { 'мне  жаль...', 2 },
                        { 'но this is ГГ and !ff', 4 }
                    },

                    {
                        { 'сука заебали меня уже все', 1.20 },
                        { 'горите в аду твари', 1.2},
                    },

                    {
                        { 'даблтап отжат, я сдох, как это???', 1.40 },
                    },

                    {
                        { 'твой батя заебал, каждый раз приходит как мышь ебанная грызет провод от интернета', 1.40 },
                    },

                    {
                        { 'ЕБАТЬ ЗА ОКНОМ ХХХТЕНТАСЁН', 1.20 },
                    },

                    {
                        { 'эзотерик здрасьте', 1.20 },
                        { 'можно разбан в ските?', 1.2},
                    },

                    {
                        { 'ага вссе спасибо', 1.20 },
                        { 'досвидания', 1.2},
                    },

                    {
                        { 'я сломал лаг комписацию', 1.20 },
                        { 'не helpануло', 1.2},
                    },

                    {
                        { 'почему у меня голова не крутится', 1.20 },
                    },

                    {
                        { 'что делать', 1.20 },
                        { 'загрузил лучшую луашку, а там flux nightly, это и есть лучшая? просто похожа', 1.2},
                    },

                    {
                        { 'сколько стоит купить такие анти - аимссс?', 1.20 },
                    },
                    ...
                    {
                        { 'как думаешь', 1.20 },
                        { 'у литвина возможно в стендофе керыч голд украсть?', 1.2 },
                    }
        ...
        local clantag do
            ...
            -- glitchy type-in / type-out of the brand
            local FLUX = {
                '/',
                '/\\',
                'f',
            ...
            local function update_flux_clantag()
                local time = math.floor(
                    globals.curtime() * 3.0
                )

                local index = time % #FLUX
                local text = FLUX[index + 1]
    ...
    local logging_system do
        ...
        local function clear_crosshair_logs()
        ...
        local function add_crosshair_log(text)
            -- FIX: only queue on-screen logs when the output is actually enabled
            if not ref.output:get 'Under crosshair' then
                return
            end
            ...
            -- FIX: new entries are inserted at the front, so the OLDEST one lives at
            -- the end. Removing index 1 deleted the log that was just added, which is
            -- why on-screen logs stopped appearing after a few shots.
            if #log_queue > 6 then
                table.remove(log_queue, #log_queue)
            end
        end

        local function print_raw(text)
            -- FIX: honour the 'Console' output option instead of always spamming console
        ...
        function _G.flux_log.dormant_hit(e)
        ...
        if crosshair_style == 'Default' then
            add_crosshair_log(gs_text)
        else
            add_crosshair_log(ae_text)
        end
    end
        function _G.flux_log.dormant_miss(e)
            local console_style = ref.console_text_style:get()
            local crosshair_style = ref.crosshair_text_style:get()
            local name = entity.get_player_name(e.userid)
        ...
        local function on_aim_hit(e)
            ...
            elseif console_style == 'New' then
                print_raw(prefixf(flux_text.raw))
                print_dev(flux_text.dev)
            end

            if crosshair_style == 'Default' then
                add_crosshair_log(gamesense_text)
            elseif crosshair_style == 'New' then
                add_crosshair_log(flux_text.log)
            end
        end
...
local function on_player_hurt(e)
        ...
        local function update_event_callbacks(value)
            if not value then
                ...
                clear_crosshair_logs()
            end
        end

        local callbacks do
            local function on_events_font(item)
                local is_modern = item:get() == 'Modern'
                local is_default = item:get() == 'Default'

                if not is_modern then
            ...
            local function on_output(item)
                ...
                local is_crosshair = item:get 'Under crosshair'
            ...
            local function on_enabled(item)
                ...
                if not value then
                    ...
                    override.unset(ref_draw_console_output)
                    -- FIX: the font callback lives on events_font, not on events
                    ref.events_font:unset_callback(on_events_font)
                end
...
trace('stage 10 - main features (rage, misc, logging)')
...
local antiaim = { } do
    ...
    local safe_head = { } do
        ...
        function safe_head:update(cmd)
            ...
            -- FIX: these were globals (always nil), so this landing check never ran
            local was_air = bit.band(localplayer.pre_flags or 0, 1) == 0
            local now_ground = bit.band(localplayer.post_flags or 0, 1) ~= 0
    ...
    local defensive = { } do
        ...
        local function update_yaw(buffer, items)
            ...
            local random = items.yaw_random:get() * 0.01

            if random > 0 then
                local yaw_offset = buffer.yaw_offset or 0
                local yaw_range = math.abs(yaw_offset) * random

                buffer.yaw_offset = yaw_offset + utils.random_int(
                    -yaw_range, yaw_range
                )

                if buffer.yaw_left ~= nil and buffer.yaw_right ~= nil then
                    local yaw_left = buffer.yaw_left
                    local yaw_right = buffer.yaw_right

                    local left_range = math.abs(yaw_left) * random
                    local right_range = math.abs(yaw_right) * random

                    buffer.yaw_left = yaw_left + utils.random_int(
                        -left_range, left_range
                    )
                    buffer.yaw_right = yaw_right + utils.random_int(
        ...
        function defensive:apply(cmd, items)
            ...
            if buffer_ctx.body_yaw ~= 'Off'
                and buffer_ctx.body_yaw ~= 'Opposite' then
                local random = items.body_yaw_random:get() * 0.01
                local body_yaw_offset = buffer_ctx.body_yaw_offset or 0
                local range = math.abs(body_yaw_offset) * random

                if range > 0 then
                    buffer_ctx.body_yaw_offset = body_yaw_offset
                        + utils.random_int(-range, range)
                end
            end
    ...
    local fakelag_clone = { } do
        local ref = resource.antiaim.fakelag
        local max_process_ticks_ref = ui.reference(
            'Misc', 'Settings', 'sv_maxusrcmdprocessticks2'
        )
        local max_process_ticks_cvar = cvar.sv_maxusrcmdprocessticks
        ...
        ui.set_visible(max_process_ticks_ref, false)

        function fakelag_clone:update(cmd)
            local is_active = is_active()
            local max_process_ticks = ref.limit:get() + 1
            local choke_limit = ref.limit:get()

            if ref.unsafe_choke:get() and is_active then
                max_process_ticks = max_process_ticks + 1
                choke_limit = choke_limit + 1
            end

            if max_process_ticks == 15 then
                max_process_ticks = 16
            end

            override.set(max_process_ticks_ref, max_process_ticks)

            if max_process_ticks_cvar ~= nil then
                max_process_ticks_cvar:set_int(max_process_ticks, true)
            end
            ...
            if ref.force_choke:get() and is_active and cmd ~= nil
                and cmd.chokedcommands < choke_limit then
        ...
        function fakelag_clone:shutdown()
            local max_process_ticks = override.get(max_process_ticks_ref)
            ...
            override.unset(max_process_ticks_ref)

            if max_process_ticks ~= nil and max_process_ticks_cvar ~= nil then
                max_process_ticks_cvar:set_int(max_process_ticks, true)
            end
        end
    ...
    local builder = { } do
        ...
        local function get_enabled_direction(items)
            if items == nil or items.yaw_direction == nil then
                return nil
            end

            local direction = get_move_direction(localplayer.move_dir)
            local enabled = items['enabled_dir_' .. direction]

            if enabled == nil or not enabled:get() then
                return nil
            end
        ...
        local function update_yaw(items)
            ...
            local direction = get_enabled_direction(items)

            if direction ~= nil then
                local item_yaw_left = items['yaw_left_dir_' .. direction]
                local item_yaw_right = items['yaw_right_dir_' .. direction]
        ...
        function builder:get_defensive_items(items)
            if items == nil or items.defensive == nil then
                return nil
            end

            local directions = items.defensive_directions
            local direction = get_enabled_direction(items.angles)

            if directions == nil or direction == nil then
                return items.defensive
            end

            return directions:get_proxy(direction, items.defensive)
                or items.defensive
        end
        ...
        local function update_freestanding_options(cmd, team)
            ...
            if localplayer.is_vulnerable then
                if items ~= nil and items.defensive ~= nil then
                    if defensive:apply(cmd, builder:get_defensive_items(items)) then
    ...
    local antiaim_on_use = { } do
        ...
        function antiaim_on_use:update(cmd, team)
            ...
            if items ~= nil and items.defensive ~= nil then
                defensive:apply(cmd, builder:get_defensive_items(items))
            end
            ...
            if state and items ~= nil then
                if items.defensive ~= nil then
                    local applied = defensive:apply(
                        cmd, builder:get_defensive_items(items)
                    )
    ...
    local function update_yaw_offset()
        if buffer.yaw_left == nil or buffer.yaw_right == nil then
            return
        end

        -- FIX: the side used to be taken from body_yaw_offset only. With
        -- Body yaw = Off / Opposite (or a 0 offset) neither branch ever ran, so the
        -- Yaw left / Yaw right sliders were silently ignored and the builder looked
        -- like it did not apply anything. Fall back to the internal inverter side.
        local side = 0

        local body_yaw_offset = buffer.body_yaw_offset or 0

        if body_yaw_offset < 0 then
            side = -1
        elseif body_yaw_offset > 0 then
            side = 1
        else
            side = inverted and -1 or 1
        end
...
local render do
    local changers do
        ...
        local viewmodel do
            ...
            -- FIX: this used to read an undefined global and never store anything, so the
            -- "weapon changed" branch fired on every single frame
            local old_weapon_index = nil
    ...
    local user_interface do
        local watermark do
            ...
            local callbacks do
                local function on_select(item)
                    -- FIX: the multiselect options are Full / Compact / Split,
                    -- the old code asked for Default / Simple / Alternative, so
                    -- no paint callback was ever registered (watermark invisible)
                    utils.event_callback('paint_ui', default.paint_ui, item:get 'Full')
                    utils.event_callback('paint_ui', simple.paint_ui, item:get 'Compact')
                    utils.event_callback('paint_ui', alt.paint_ui, item:get 'Split')
                end
            ...
            local on_paint_ui do
                ...
                local function draw_stars(position, r1, g1, b1, a1, r2, g2, b2, a2)
                    ...
                    for i = 1, len do
                        ...
                        if style == 'Cosmic' then
                            text = text_anims.cosmic(
                                text, (time + pct) * 0.5,
                                0.5, 0.4, 1.0, 0.5
                            )
                        end
        ...
        local real_yaw_arrows do
            local ref = resource.antiaim.hotkeys.real_yaw_arrows

            local yaw = 0

            local function get_real_yaw()
                local me = entity.get_local_player()
                ...
                return utils.normalize(eye_yaw, -180, 180)
            end
            ...
            local function on_paint_ui()
                local real_yaw = get_real_yaw()

                if real_yaw == nil then
                    return
                end
                ...
                local delta = utils.normalize(real_yaw - yaw, -180, 180)
                local speed = math.min(globals.frametime() * 12, 1)

                yaw = utils.normalize(yaw + delta * speed, -180, 180)

                local radius = ref.offset:get()
                local point_x, point_y = rotate(radius, 0, yaw)
                local x = center.x + point_x
                local y = center.y + point_y
                local color_arrow = color(ref.color:get())

                -- Exact Modern ⮜ outline: an open chevron, not a triangle.
                -- The original glyph points left, so rotate it 180 degrees
                -- beyond the head yaw to make its tip point outward.
                local angle = yaw + 180
                local tip_x, tip_y = rotate(-10.0156, 0, angle)
                local top_x, top_y = rotate(10.0156, -11.5625, angle)
                local notch_x, notch_y = rotate(3.3438, 0, angle)
                local bottom_x, bottom_y = rotate(10.0156, 11.5625, angle)
...
trace('stage 11 - anti-aim')

-- ---------------------------------------------------------------- startup splash
-- plain text instead of the old embedded jpeg
do
    local duration      = 4
    ...
    local function draw_vignette(screen_width, screen_height, opacity)
        local background_alpha = math.floor(95 * opacity)
        local edge_alpha = math.floor(90 * opacity)
        local edge_width = math.floor(screen_width * 0.3)
        local edge_height = math.floor(screen_height * 0.3)

        renderer.rectangle(0, 0, screen_width, screen_height, 0, 0, 0, background_alpha)
        renderer.gradient(0, 0, edge_width, screen_height, 0, 0, 0, edge_alpha, 0, 0, 0, 0, true)
        renderer.gradient(screen_width - edge_width, 0, edge_width, screen_height, 0, 0, 0, 0, 0, 0, 0, edge_alpha, true)
        renderer.gradient(0, 0, screen_width, edge_height, 0, 0, 0, edge_alpha, 0, 0, 0, 0, false)
        renderer.gradient(0, screen_height - edge_height, screen_width, edge_height, 0, 0, 0, 0, 0, 0, 0, edge_alpha, false)
    end

    local function draw_startup_text()
        local elapsed = globals.realtime() - started_at

        if elapsed >= duration then
            client.unset_event_callback('paint_ui', draw_startup_text)
            return
        end
        ...
        if elapsed < fade_duration then
            alpha = math.floor(255 * elapsed / fade_duration)
        elseif elapsed > duration - fade_duration then
            alpha = math.floor(255 * (duration - elapsed) / fade_duration)
        end

        alpha = math.max(0, math.min(255, alpha))

        local screen_width, screen_height = client.screen_size()
        draw_vignette(screen_width, screen_height, alpha / 255)

        local ar, ag, ab = software.get_color()
        local x, y = screen_width * 0.5, screen_height * 0.5 - 30

        -- accent rule above the block
        renderer.gradient(x - 90, y - 12, 90, 1, ar, ag, ab, 0, ar, ag, ab, alpha, true)
        renderer.gradient(x, y - 12, 90, 1, ar, ag, ab, alpha, ar, ag, ab, 0, true)

        for i = 1, #lines do
            local line = lines[i]

            if line.accent then
                renderer.text(x, y, ar, ag, ab, alpha, 'c' .. line.size, 0, line.text)
            else
                renderer.text(x, y, 235, 235, 240, alpha, 'c' .. line.size, 0, line.text)
            end
    ...
    client.set_event_callback('paint_ui', draw_startup_text)
end

trace('stage 12 - ready')
